...
function fenye(data){
                 ...
                 setpage();//调用分页函数
                 for(var i=0;i<data.renwus.length;i++){  //第一次返回，最多返回6条数据
                         var tds=document.getElementById('td'+i+0);                                      
                             tds.innerHTML=data.renwus[i].stime; 
                         for(var j=0;j<2;j++){
                            var tds=document.getElementById('td'+i+1);                                        
                             tds.innerHTML=data.renwus[i].ttitle;           
                         }
                         for(var k=0;k<2;k++){
                            var tds=document.getElementById('td'+i+2);                                        
                             tds.innerHTML=data.renwus[i].taddress;           
                         }
                         var tds=document.getElementById('td'+i+3);                          
                           ...
                           //  as.innerHTML='详情';
                            ...
                            tds.innerHTML='';
                            tds.appendChild(as);
                            var col = new cx(data.renwus[i].id);//调用回调函数
                            // alert(num);
